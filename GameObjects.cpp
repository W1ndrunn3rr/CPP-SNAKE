...
//! Generator liczb losowych, do przydzielania jabłka w losowych miejscach na planszy
int generateNumber(int height, int width, int mode) {
    //Generator liczb losowych zapożyczony ze Stack overflow
    if (mode == 1) {
        std::random_device dev;
        std::mt19937 rng(dev());
        std::uniform_int_distribution<std::mt19937::result_type> randomRow(1, height - 2); // zakres zmniejszony
        return int(randomRow(rng));
    }
    if (mode == 2) {
        std::random_device dev;
        std::mt19937 rng(dev());
        std::uniform_int_distribution<std::mt19937::result_type> randomCol(1, width - 2);
...
bool GameObjects::Collision() {
    for (int snakeElement = 1; snakeElement <
                               int(snake.size()); snakeElement++) {
    ...
    if (snake.front().colCord == 0 || snake.front().rowCord == 0 || snake.front().rowCord == height - 1 ||
...
void GameObjects::eatApple() {
    if (snake.front().colCord == apple.colCord && snake.front().rowCord == apple.rowCord) {
        placeApple();
...
int GameObjects::getScore() const {
    return score;
}


GameState GameObjects::getGameState() {
    return state;
}

void GameObjects::setScorePoints(int points) {
   scorePoints = points;

}

std::vector<Cords> GameObjects::getSnake() {
    return snake;
}
...
void GameObjects::placeApple() {
    ...
    for (auto &&element: snake) {
        while (element.rowCord == apple.rowCord && element.colCord == apple.colCord) {
            apple.rowCord = generateNumber(height, width, 1);
            apple.colCord = generateNumber(height, width, 2);
        }
    }
...
Cords GameObjects::getApple() const {
    return apple;
}
