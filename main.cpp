...
int main() {
    sf::Event event{};
    sf::RenderWindow window(sf::VideoMode(900, 900), "Snake", sf::Style::Titlebar);
    ...
    while (window.isOpen()) {
        ...
        if (menuController.getDifficultyChoice() == 'C') {
            menu.drawMenu(window, menuController.handleMenuEvent(event), menuController.handleLevelEvent(event));
        } else {
            window.close();
        }
    ...
    if (menuController.getClick() == 'P') {
        Board board(22, 22);
        sf::RenderWindow newWindow(sf::VideoMode(900, 900), "SNAKE", sf::Style::Titlebar);
